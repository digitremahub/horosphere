...
/** Titre + contenu du dernier article publié — sert de point de comparaison
 * pour que chaque nouvel article s'organise autour d'une vraie période de
 * changement (nouvelle phase de lune, nouveau signe solaire, début/fin de
 * rétrogradation, nouvel événement) plutôt que de remettre en avant un fait
 * déjà couvert et toujours d'actualité (ex. une rétrogradation qui dure des
 * mois). Retombe sur `null` si la base est indisponible plutôt que de
...
} Termine l'article par un court paragraphe ou une liste nommant ces 3 signes et l'influence de leur aspect (conjonction = concerné en premier, opposition = tension à équilibrer, carré = friction qui pousse à ajuster) — sans inventer d'autres signes ni d'autres aspects que ceux donnés. Ne prétends jamais calculer un ascendant, une maison ou un transit précis non fourni ci-dessus — reste sur les données données. Ton : chaleureux, curieux, jamais fataliste ni anxiogène, un peu poétique sans être vague — Horosphère aborde l'astrologie comme un outil de développement personnel : explique la cohérence du ciel (le "pourquoi"), et fais déboucher chaque section sur ce que ça change concrètement pour le lecteur cette semaine. Longueur : 200 à 300 mots pour le contenu.
