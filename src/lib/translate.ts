...
export async function translatedTitles(items: TranslatableNews[], locale: NewsLocale): Promise<Map<string, string>> {
  ...
  const sql = requireDb();
  // Un titre "traduit" identique au français trahit presque toujours un
  // échec silencieux de l'IA (elle a renvoyé la langue source) plutôt qu'une
  // vraie coïncidence — on retente une fois, seulement pour les titres
  // concernés, avant d'abandonner et de retomber sur le français.
  let restants = missing;
  for (let tentative = 1; tentative <= 2 && restants.length > 0; tentative++) {
    try {
...
Réponds UNIQUEMENT avec un objet JSON au format exact {"traductions": ["titre 1 traduit", "titre 2 traduit", ...]}, dans le même ordre que la liste ci-dessous, avec exactement ${restants.length} éléments.

${restants.map((i, idx) => `${idx + 1}. ${i.titre}`).join('\n')}`;
      ...
      const echecs: TranslatableNews[] = [];
      for (let idx = 0; idx < restants.length; idx++) {
        const candidat = typeof traductions[idx] === 'string' ? (traductions[idx] as string).trim() : '';
        const echecSilencieux = !candidat || candidat === restants[idx].titre;
        if (echecSilencieux) {
          echecs.push(restants[idx]);
          continue;
        }
        result.set(restants[idx].id, candidat);
        await sql`
          INSERT INTO news_translations (news_id, locale, titre)
          VALUES (${restants[idx].id}, ${locale}, ${candidat})
          ON CONFLICT (news_id, locale) DO UPDATE SET titre = EXCLUDED.titre
        `;
      }
      if (echecs.length > 0) {
        console.error('translatedTitles: traduction ignorée (identique au français)', { newsIds: echecs.map((i) => i.id), locale, tentative });
      }
      restants = echecs;
    } catch (err) {
      console.error('translatedTitles: traduction échouée', err, { tentative });
      break;
    }
  }
  // Ce qui n'a toujours pas de traduction valide après retentative retombe
  // sur le français, sans être mis en cache.
  restants.forEach((i) => result.set(i.id, i.titre));
  return result;
}
...
${item.contenu}`;
  ...
  // français (titre ET corps) trahit presque toujours un échec silencieux de
  // l'IA plutôt qu'une vraie coïncidence — sur un texte de plusieurs phrases,
  // une traduction authentique ne retombe jamais mot pour mot sur la source.
  // On retente une fois avant d'abandonner plutôt que de retomber directement
  // sur le français (et de devoir purger le cache à la main pour se rattraper).
  for (let tentative = 1; tentative <= 2; tentative++) {
    try {
      ...
      if (traduit.titre === item.titre && traduit.contenu === item.contenu) {
        console.error('translatedArticle: traduction ignorée (identique au français)', { newsId: item.id, locale, tentative });
        continue;
      }
      const sql = requireDb();
      await sql`
      ...
      console.error('translatedArticle: traduction échouée', err, { tentative });
    }
  }
  return item;
}
